...
COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    ...
    ["💲 Prime Plans", "⭐ VIP Plans"],
    ["📞 Contact Owner", "🤖 ChatGPT"]
]
...
ADMIN_COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    ...
    ["🤖 ChatGPT", "⏱ Uptime"],
    ["📂 Global History", "💲 Prime Plans"],
    ["⭐ VIP Plans", "🖥️ Bot Manager"],
    ["📊 System Stats", "👥 All Users Bots"]
]
...
def create_main_menu_inline(user_id):
    ...
    if user_id in admin_ids:
        admin_buttons = [
            ...
            types.InlineKeyboardButton('👥 All Users Bots', callback_data='all_users_bots')
        ]
...
def create_admin_panel():
    ...
    markup.row(types.InlineKeyboardButton('💲 Prime Plans', callback_data='show_prime_plans'))
    markup.row(types.InlineKeyboardButton('⭐ VIP Plans', callback_data='show_vip_plans'))
...
def show_prime_plans(chat_id, message_id=None):
    text = """🥇 **PRIME PLANS** 🥇
...
**Select a plan below:**"""
    ...
    markup = create_prime_plans()
    
    if message_id:
        try:
            bot.edit_message_text(text, chat_id, message_id, reply_markup=markup, parse_mode='Markdown')
        except Exception as e:
            logger.error(f"Error editing prime plans: {e}")
            bot.send_message(chat_id, text, reply_markup=markup, parse_mode='Markdown')
    else:
        bot.send_message(chat_id, text, reply_markup=markup, parse_mode='Markdown')

def show_vip_plans(chat_id, message_id=None):
    text = """⭐ **VIP PLANS** ⭐
...
**Select a plan below:**"""
    ...
    markup = create_vip_plans()
    
    if message_id:
        try:
            bot.edit_message_text(text, chat_id, message_id, reply_markup=markup, parse_mode='Markdown')
        except Exception as e:
            logger.error(f"Error editing vip plans: {e}")
            bot.send_message(chat_id, text, reply_markup=markup, parse_mode='Markdown')
    else:
        bot.send_message(chat_id, text, reply_markup=markup, parse_mode='Markdown')
...
@bot.message_handler(func=lambda message: message.text == "⭐ VIP Plans")
def vip_button_handler(message):
    show_vip_plans(message.chat.id)
...
def admin_commands(message):
    ...
    if len(args) < 2:
        bot.reply_to(message, "📋 **Admin Commands:**\n"
                             "/admin prime <id> <days> - Add Prime\n"
                             "/admin vip <id> <days> - Add VIP\n"
                             "/admin ban <id> - Ban user\n"
                             "/admin unban <id> - Unban user\n"
                             "/admin host <id> <time> - Add hosting\n"
    ...
    if command == "prime":
        if len(args) < 4:
            bot.reply_to(message, "Usage: /admin prime <user_id> <days> e.g., /admin prime 123456789 30")
            return
        days = int(args[3])
        ...
        if len(args) < 4:
            bot.reply_to(message, "Usage: /admin vip <user_id> <days> e.g., /admin vip 123456789 30")
            return
        days = int(args[3])
        ...
        if len(args) < 4:
            bot.reply_to(message, "Usage: /admin host <user_id> <time> e.g., /admin host 123456789 24h")
            return
        time_str = args[3]
        add_hosting(user_id, time_str)
        bot.reply_to(message, f"✅ Hosting added to user {user_id} for {time_str}.")
    elif command == "remove":
        if user_id is None:
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "💲 Prime Plans": lambda m: show_prime_plans(m.chat.id),
    "⭐ VIP Plans": lambda m: show_vip_plans(m.chat.id),
    ...
    "👥 All Users Bots": _logic_all_users_bots
}
...
def handle_callbacks(call):
    ...
    try:
        if data.startswith('pay_done_'):
            parts = data.split('_')
            # Format: pay_done_{plan_type}_{days}_{plan_name}
            plan_type = parts[2]
            days = int(parts[3])
            plan_name = '_'.join(parts[4:])
            
            if plan_type == "prime":
                price = PRIME_PLANS.get(plan_name, {}).get('price', '₹0')
                if not price:
                    for name, info in PRIME_PLANS.items():
                        if info['days'] == days:
                            plan_name = name
                ...
                price = VIP_PLANS.get(plan_name, {}).get('price', '₹0')
                if not price:
                    for name, info in VIP_PLANS.items():
                        if info['days'] == days:
                            plan_name = name
...
def add_hosting(user_id, time_str):
    ...
    with DB_LOCK:
        ...
        try:
            c.execute('INSERT OR REPLACE INTO user_hosting (user_id, hosting_time, file_count) VALUES (?, ?, ?)',
                      (user_id, expiry.isoformat(), 10))
            conn.commit()
            user_hosting[user_id] = {'hosting_time': expiry, 'file_count': 10}
            logger.info(f"Added hosting for {user_id} until {expiry.isoformat()} with 10 files")
